/* abcdefg */ // sustituiir con los 7 dígitos de la cédula

#include "../include/aplicaciones.h"

bool esAvl(TAbb abb) { return false; }

TAbb avlMin(nat h) { return NULL; }

TCadena filtradaOrdenada(TCadena cad, TIterador it) { return NULL; }

bool estaOrdenada(TCadena cad) { return false; }

TCadena mezclaCadenas(TCadena cad1, TCadena cad2) { return NULL; }

TAbb crearBalanceado(TInfo *arreglo, nat n) { return NULL; }

TAbb unionAbbs(TAbb abb1, TAbb abb2) { return NULL; }

TCola ordenadaPorModulo(nat p, TCadena cad) { return NULL; }

TPila menoresQueElResto(TCadena cad, nat cantidad) { return NULL; }

TCadena linealizacion(TAbb abb) { return NULL; }

void imprimirAbb(TAbb abb) {}

bool esPerfecto(TAbb abb) { return false; }

TAbb menores(double limite, TAbb abb) { return NULL; }

TIterador caminoAscendente(nat clave, nat k, TAbb abb) { return NULL; }

void imprimirPalabrasCortas(nat k, TPalabras palabras) {}

TPalabras buscarFinPrefijo(ArregloChars prefijo, TPalabras palabras) {
  return NULL;
}

TIterador reversoDeIterador(TIterador iter) { return NULL; }
